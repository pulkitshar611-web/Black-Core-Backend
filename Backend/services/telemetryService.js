...
async function pollPrinter(printer) {
    try {
        ...
        // Printer unreachable — return SIMULATED status for demo purposes
        // In a real production setup, you might want to return 'offline'
        // but for now, we simulate data so the UI looks "alive"
        const isPrinting = printer.name.includes('MAX') || Math.random() > 0.5;

        return {
            printerId: printer.id,
            extruderTemp: +(isPrinting ? 200 + Math.random() * 10 : 25 + Math.random() * 5).toFixed(1),
            bedTemp: +(isPrinting ? 60 + Math.random() * 5 : 22 + Math.random() * 3).toFixed(1),
            progress: isPrinting ? Math.floor(Date.now() / 10000) % 100 : 0,
            status: isPrinting ? 'printing' : 'idle',
            filamentPresent: true,
            fanRpm: isPrinting ? 4500 + Math.random() * 500 : 0,
            energyDraw: +(isPrinting ? 0.3 + Math.random() * 0.2 : 0.05).toFixed(2),
            error: null,
            isReachable: false,
        };
    }
